...
+ (void)avoidCrashExchangeMethod {
    ...
    dispatch_once(&onceToken, ^{
        ...
        // https://bugly.qq.com/v2/crash-reporting/crashes/899142f08e/58884?pid=2
        // NLog在iOS11的闪退，通过此方式尝试处理
        if ([[UIDevice currentDevice].systemVersion floatValue] >= 11 &&
            [[UIDevice currentDevice].systemVersion floatValue] < 12) {
            [AvoidCrash exchangeInstanceMethod:[self class] method1Sel:@selector(initWithArray:range:copyItems:) method2Sel:@selector(avoidCrashInitWithArray:range:copyItems:)];
        }
...
- (instancetype)avoidCrashInitWithArray:(NSArray *)array range:(NSRange)range copyItems:(BOOL)flag {
    id object = nil;
    
    @try {
        object = [self avoidCrashInitWithArray:array range:range copyItems:flag];
    }
    @catch (NSException *exception) {
